...
# ─────────────────────────────────────────────
def send_instagram_alert(alert_type: str, odds: dict, api_key: str, user_id: str):
    """
    Sends alert via Instagram Graph API (Direct Message or Story).
    You need: Business/Creator account + approved permissions.

    Replace with your preferred notification method if needed
    (Telegram, WhatsApp, push notification, etc.)
    """
    if not api_key or not user_id:
        return False, "Instagram API key or User ID not set."
    ...
    # ── Instagram Graph API — Messages endpoint ──
    # Requires instagram_messaging permission approved by Meta.
    #
    # url = f"https://graph.instagram.com/v19.0/{user_id}/messages"
    # payload = {
...
with st.sidebar:
    ...
    st.session_state.instagram_api_key = st.text_input(
        "Instagram Access Token",
        value=st.session_state.instagram_api_key,
        type="password",
        placeholder="sua_chave_aqui"
    )
    st.session_state.instagram_user_id = st.text_input(
        "Instagram User ID",
        value=st.session_state.instagram_user_id,
        placeholder="seu_user_id"
    )
...
if st.session_state.monitoring:
    ...
    try:
        ...
        for alert_type in triggered:
            ok, response = send_instagram_alert(
                alert_type=alert_type,
                odds=raw,
                api_key=st.session_state.instagram_api_key,
                user_id=st.session_state.instagram_user_id,
            )
            ...
            log_entry = {
                ...
                "msg":  (
                    ...
                    f"| {'✓ Instagram sent' if ok else '✗ Instagram fail'}"
                ),
            }
